...
import {
  ...
  incorrectUserData,
} from './dataForTest/dataTestforUser';
...
describe('/user', () => {
  ...
  it(`shouldn't create user with empty login`, async () => {
    ...
    await usersTestManager.createUserAdmin(data, HttpStatus.BAD_REQUEST, 1);
  ...
  it(`shouldn't create user with incorrect data`, async () => {
    ...
    await usersTestManager.createUserAdmin(data, HttpStatus.BAD_REQUEST, 3);
  ...
  it('should return users "asc" ', async () => {
    ...
    await request(httpServer)
      .get(`${RouterPaths.usersSa}/?pageSize=15&sortDirection=asc`)
      .set('authorization', 'Basic YWRtaW46cXdlcnR5')
      .expect(HttpStatus.OK, {
        pagesCount: 1,
        page: 1,
  ...
  it('should return user with "og9" in login ', async () => {
    ...
    await request(httpServer)
      .get(`${RouterPaths.usersSa}/?searchLoginTerm=og9`)
      .set('authorization', 'Basic YWRtaW46cXdlcnR5')
      .expect(HttpStatus.OK, {
        pagesCount: 1,
        page: 1,
  ...
  it('should return user with "il8" in email ', async () => {
    ...
    await request(httpServer)
      .get(`${RouterPaths.usersSa}/?searchEmailTerm=il8`)
      .set('authorization', 'Basic YWRtaW46cXdlcnR5')
      .expect(HttpStatus.OK, {
        pagesCount: 1,
        page: 1,
  ...
  it(`shouldn't delete  user UNAUTHORIZED`, async () => {
    ...
    await request(httpServer)
      .delete(`${RouterPaths.usersSa}/${user.createdEntity.id}`)
      .set('authorization', 'Basic YWRtaW')
      .expect(HttpStatus.UNAUTHORIZED);
  });
  ...
  it(`shouldn't delete  user`, async () => {
    await request(httpServer)
      .delete(`${RouterPaths.usersSa}/7779161`)
      .set('authorization', 'Basic YWRtaW46cXdlcnR5')
  ...
  it(`should delete both user`, async () => {
    ...
    await request(httpServer)
      .get(`${RouterPaths.usersSa}/${user.createdEntity.id}`)
      .set('authorization', 'Basic YWRtaW46cXdlcnR5')
      .expect(HttpStatus.NOT_FOUND);
